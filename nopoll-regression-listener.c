...
void *listener_for_URL_redirection (void *vargp) {
	...
	char server_response2[] = "HTTP/1.1 500 Temporary Redirect\r\n\r\n";
	
	char  * reply;
    int listener, client_sock1, client_sock2;
	...
	{
		if (!client_conn){
			 client_sock1 = accept(listener, (struct sockaddr*) NULL, NULL);
			...
			if(write(client_sock1, reply, strlen(reply)+1)){
			...
			/* send accept header accepting protocol requested by the user */
			 client_sock2 = accept(listener, (struct sockaddr*) NULL, NULL);
			reply = server_response2;
			printf("INFO: Received request at %s:%d, replying to client %s",inet_ntoa(serveraddr.sin_addr), htons(serveraddr.sin_port),reply);
	 
			if(write(client_sock2, reply, strlen(reply)+1)){
				return NULL;
	    	}
		}
